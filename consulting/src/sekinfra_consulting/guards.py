...
IMPLEMENTATION_OUTCOME_TRANSITIONS=frozenset({"RecordImplementationOutcomeApproval","ApproveImplementationOutcome","RevokeImplementationOutcome"})
...
class GuardPipeline:
    ...
    def subject(self,p,c,s,t):
        ...
        if p.command_type in IMPLEMENTATION_OUTCOME_TRANSITIONS and (not s or not s.exists or s.subject_type!=p.subject_type or s.subject_id!=p.subject_id):
            return self.fail(RuntimeReason.TENANT_SUBJECT_MISMATCH,"authoritative ImplementationOutcome is required and must match","subject")
    ...
    def version(self,p,c,s,t):
        ...
        if p.command_type in IMPLEMENTATION_OUTCOME_TRANSITIONS and (not s or p.expected_record_version!=s.record_version):
            return self.fail(RuntimeReason.VERSION_STALE,"authoritative ImplementationOutcome record version is stale","version")
