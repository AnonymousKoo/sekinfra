...
from .postgres_implementation_outcome import ImplementationOutcomePostgresRepository
...
class PostgresStore:
    ...
    def snapshot(self, command, trusted_context=None):
        queries = {
            ...
            "IMPLEMENTATION_OUTCOME": ("select tenant_id,record_version,engagement_id,state from public.sekinfra_implementation_outcomes where tenant_id=%s and implementation_outcome_id=%s and state<>'SUPERSEDED' order by outcome_version desc limit 1", "state"),
...
class HumanApprovalPostgresRepository(_TenantRepository):
    ...
    def get(self, tenant_id, approval_id):
        ...
        if row.get("subject_type") == "IMPLEMENTATION_OUTCOME":
            return self._implementation_outcome(row)
    ...
    def _implementation_outcome(self,r):
        return {"approval_id":str(r["approval_id"]),"tenant_id":str(r["tenant_id"]),"engagement_id":str(r["engagement_id"]),"subject_type":r["subject_type"],"subject_id":str(r["subject_id"]),"subject_version":r["subject_version"],"approval_category":r["approval_category"],"authority_category":r["authority_category"],"actor_identity":r["actor_identity"],"actor_organization":r["actor_organization"],"actor_role":r["actor_role"],"decision":r["decision"],"implementation_outcome_authority":{"subject_id":str(r["subject_id"]),"authority_digest":r["implementation_outcome_authority_digest"]},"conditions":_load(r["conditions"]) or [],"effective_at":_time(r["effective_at"]),"evidence_reference":_load(r["evidence_reference"]),"status":r["status"],"correlation_id":str(r["correlation_id"]),"idempotency_key":r["idempotency_key"],"created_at":_time(r["created_at"])}
    def find_active_implementation_outcome_binding(self,tenant_id,outcome_id,outcome_version,digest,authority_role):
        row=self.uow.connection.execute("select * from public.sekinfra_human_approvals where tenant_id=%s and subject_type='IMPLEMENTATION_OUTCOME' and subject_id=%s and subject_version=%s and implementation_outcome_authority_digest=%s and actor_role=%s and status='ACTIVE'",(tenant_id,outcome_id,outcome_version,digest,authority_role)).fetchone()
        return self._implementation_outcome(row) if row else None
    def record_implementation_outcome(self,record):
        self.uow.failpoint("AUTHORITATIVE_WRITE")
        cur=self.uow.connection.execute(
            "insert into public.sekinfra_human_approvals (approval_id,tenant_id,engagement_id,approval_role,authority_category,approving_principal_reference,approving_organization_reference,decision,status,conditions,effective_at,evidence_reference,correlation_id,idempotency_key,subject_type,subject_id,subject_version,approval_category,actor_identity,actor_organization,actor_role,implementation_outcome_authority_digest,created_at) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s::jsonb,%s,%s,%s,%s,'IMPLEMENTATION_OUTCOME',%s,%s,'IMPLEMENTATION_OUTCOME',%s,%s,%s,%s,%s) on conflict do nothing returning approval_id",
            (record["approval_id"],record["tenant_id"],record["engagement_id"],record["actor_role"],record["authority_category"],record["actor_identity"],record["actor_organization"],record["decision"],record["status"],_json(record["conditions"]),record["effective_at"],_json(record["evidence_reference"]),record["correlation_id"],record["idempotency_key"],record["subject_id"],record["subject_version"],record["actor_identity"],record["actor_organization"],record["actor_role"],record["implementation_outcome_authority"]["authority_digest"],record["created_at"]),
        )
        if not cur.fetchone():raise ValueError("duplicate active implementation outcome authority")
        return copy.deepcopy(record)
