...
create table public.sekinfra_idempotency_records (
  ...
  constraint sekinfra_idempotency_records_tenant_id_trusted_principal_id_co_key unique (tenant_id, trusted_principal_id, command_type, subject_type, subject_id, idempotency_key), check (processing_status not in ('COMPLETED','FAILED_TERMINAL') or completed_at is not null), check (processing_status <> 'COMPLETED' or result_reference is not null)
);
